...
func testUpdateNodeRoute(t *testing.T, family string) {
	...
	lnh := newNodeHandler(log, dpConfig, nodemapfake.NewFakeNodeMapV2(), kpr.KPRConfig{}, ipsec.NewTestIPsecAgent(t), fakeipsec.Config{}, lns, nil)
...
func testNodeUpdateIDs(t *testing.T, family string) {
	...
	log := hivetest.Logger(t)
	lns := node.NewTestLocalNodeStore(node.LocalNode{})
	lnh := newNodeHandler(log, dpConfig, nodeMap, kpr.KPRConfig{}, ipsec.NewTestIPsecAgent(t), fakeipsec.Config{}, lns, nil)
...
func testNodeChurnXFRMLeaksWithConfig(t *testing.T, s *nodeSuite, config config.Config) {
	...
	dpConfig := DatapathConfiguration{HostDevice: hostDevice}
	lns := node.NewTestLocalNodeStore(node.LocalNode{})
	lnh := newNodeHandler(log, dpConfig, nodemapfake.NewFakeNodeMapV2(), kpr.KPRConfig{}, a, fakeipsec.Config{}, lns, nil)
