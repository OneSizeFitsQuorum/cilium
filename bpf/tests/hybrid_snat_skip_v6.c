 ...
 *
 * | Test Case              | src_subnet_id | dst_subnet_id | hybrid_enabled | Expected        |
 * |------------------------|---------------|---------------|----------------|-----------------|
 * | same_subnet_skip       | 100           | 100           | true           | NAT_PUNT_TO_STACK|
 * | different_subnet       | 100           | 200           | true           | NAT_NEEDED      |
 *
 * Key behaviors:
...
int test_hybrid_snat_v6_different_subnet(__maybe_unused struct __ctx_buff *ctx)
{
	struct ipv6_ct_tuple tuple __align_stack_8 = {};
	struct ipv6hdr ip6 = { .nexthdr = IPPROTO_TCP };
	fraginfo_t fraginfo = 0;
	...
	ipcache_v6_add_entry(&dst_addr, 0, REMOTE_NODE_ID, 0, 0);
	ipcache_v6_add_entry(&src_addr, 0, LOCAL_NODE_ID, 0, 0);
