...
/*
 * Test: remote_node_masquerade=true overrides hybrid SNAT skip (IPv4).
 *
 * This is a separate compilation unit from hybrid_snat_skip_v4.c because
 * ASSIGN_CONFIG can only be used once per config variable per compilation unit,
 * and this test requires enable_remote_node_masquerade=true while the other
 * tests in hybrid_snat_skip_v4.c require it to be false.
 */
...
/* Test: remote_node_masquerade=true overrides hybrid skip */
